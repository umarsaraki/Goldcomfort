...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS users (
    ...
    -- Withdrawal security code (5-digit PIN, separate from Google login).
    -- Stored salted+hashed, never in plain text. Required before any withdraw
    -- goes through, so a hijacked Google session alone can't drain a wallet.
  ...
  CREATE TABLE IF NOT EXISTS orders (
    ...
    decline_reason TEXT, -- required when the seller rejects — shown to the buyer
  ...
  CREATE TABLE IF NOT EXISTS virtual_accounts (
    ...
    account_name TEXT, -- "Full Name - GOLD COMFORT", as shown on the account
  ...
  -- Saved withdrawal bank accounts — up to 2 per person. They pick one when
  -- withdrawing, or delete one to make room for a different one.
  ...
  -- Coupons: two kinds —
  --   'signup'  = GoldComfort-issued, given to a person to encourage becoming a
  --               reseller (percent off first plan payment) OR to an existing
  --               reseller for their plan renewal/upgrade (percent off that
  --               payment). "role" says which one it's for. Can target everyone
  --               or one specific person, and can optionally expire.
  --   'product' = a reseller's own discount code on their own listing(s)
  CREATE TABLE IF NOT EXISTS coupons (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    code TEXT NOT NULL UNIQUE,
    type TEXT NOT NULL, -- 'signup' | 'product'
    role TEXT NOT NULL DEFAULT 'customer', -- 'customer' | 'reseller' (signup coupons only)
    reseller_id INTEGER,
    discount_percent INTEGER,
  ...
  -- Each slide can carry a link (e.g. a paid ad's landing page) and tracks its
  -- own view/click counts, so admin can show a real advertiser real numbers
  -- without needing any external analytics tool.
  CREATE TABLE IF NOT EXISTS banner_slides (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    image TEXT, -- data URL, nullable
    text TEXT, -- nullable
    link_url TEXT, -- nullable — opens when the slide is tapped
  ...
  -- Cashback (auto-granted on delivery) uses this same table — reason starts
  -- with 'Cashback' — and always carries an expires_at; admin-granted credits
  -- usually don't.
