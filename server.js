...
// own Flutterwave business name ("Umar Idris"). Pass the real person's name here
// and Flutterwave shows THEIR name on the account, every time.
...
function flutterwaveCreateVirtualAccount(ownerEmail, ownerFullName, ninOrBvn) {
  const [firstname, ...rest] = (ownerFullName || 'GoldComfort User').trim().split(' ');
  const lastname = rest.join(' ') || firstname;
  const narration = `GoldComfort — ${ownerFullName || ownerEmail}`;
  // TODO once live: fetch(...) to Flutterwave with { email: ownerEmail, is_permanent: true,
  //   bvn: ninOrBvn, tx_ref: 'VA-'+crypto.randomUUID(), firstname, lastname, narration }
  const fakeAcct = '90' + String(Math.floor(10000000 + Math.random() * 89999999));
  return { success: true, accountNumber: fakeAcct, bankName: 'Flutterwave-Titan MFB', firstname, lastname, narration };
}
...
function getPendingBalance(ownerType, ownerId) {
  ...
  if (ownerType === 'admin') {
    const row = db.prepare(`SELECT COALESCE(SUM(commission),0) p FROM orders WHERE status IN ('preparing','on-way','arrived')`).get();
    return row.p;
  }
  return 0;
}
...
route('POST', '/api/orders/:id/confirm', async (req, res, body, params) => {
  ...
  sendJSON(res, 200, { status: 'delivered', payout: order.reseller_payout });
});
...
route('POST', '/api/coupons', async (req, res, body) => {
  try {
    const info = db.prepare(`
      INSERT INTO coupons (code, type, reseller_id, discount_percent, audience, target_email, expires_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(
      body.code.trim().toUpperCase(), body.type, body.resellerId || null, body.discountPercent,
...
route('POST', '/api/banner-slides', async (req, res, body) => {
  ...
  const info = db.prepare('INSERT INTO banner_slides (image, text, sort_order) VALUES (?, ?, ?)')
    .run(body.image || null, body.text || null, maxOrder + 1);
  sendJSON(res, 201, { id: info.lastInsertRowid });
});
...
route('GET', '/api/wallet/:ownerType/:ownerId', async (req, res, body, params) => {
  ...
  const available = getWalletBalance(ownerType === 'admin' ? 'admin' : ownerType, decodeURIComponent(ownerId));
  ...
  sendJSON(res, 200, { available, pending, totalEarning });
});
...
route('POST', '/api/wallet/:ownerType/:ownerId/virtual-account', async (req, res, body, params) => {
  ...
  const result = flutterwaveCreateVirtualAccount(body.email, realName, body.nin || body.bvn);
  if (!result.success) return sendJSON(res, 502, { error: 'Could not generate account number — please try again' });

  db.prepare('INSERT INTO virtual_accounts (owner_type, owner_id, account_number, bank_name) VALUES (?, ?, ?, ?)')
    .run(ownerType, decodedId, result.accountNumber, result.bankName);
...
route('POST', '/api/wallet/:ownerType/:ownerId/withdraw', async (req, res, body, params) => {
  ...
  // Save these bank details as the default for next time.
  if (ownerType === 'customer' || ownerType === 'admin') {
    db.prepare('UPDATE users SET withdraw_bank_name = ?, withdraw_bank_acct = ?, withdraw_bank_acct_name = ? WHERE email = ?')
      .run(bankDetails.bankName, bankDetails.bankAcct, bankDetails.bankAcctName, decodedId);
  } else if (ownerType === 'reseller' && body.saveAsDefault) {
    db.prepare('UPDATE resellers SET bank_name = ?, bank_acct = ?, bank_acct_name = ? WHERE id = ?')
      .run(bankDetails.bankName, bankDetails.bankAcct, bankDetails.bankAcctName, decodedId);
  }
