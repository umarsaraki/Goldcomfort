...
route('POST', '/api/orders', async (req, res, body) => {
  ...
  if (body.payWithWallet && total > getWalletBalance('customer', body.buyerEmail)) {
  ...
  if (body.payWithWallet && total > 0) {
    try {
      debitWallet('customer', body.buyerEmail, total, `Order payment — #${info.lastInsertRowid}`, info.lastInsertRowid);
    } catch (err) {
...
route('GET', '/api/wallet/:ownerType/:ownerId', async (req, res, body, params) => {
  ...
  sendJSON(res, 200, { available, pending, totalEarning, totalBalance, totalRefunded });
});
