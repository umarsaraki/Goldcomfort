...
// with { email, bvn or is_permanent:true + tx_ref, narration }. Returns a real
// account_number + bank_name that Flutterwave watches forever — any transfer to
// it fires a webhook (tx_ref will need a TOPUP- prefix so our router recognizes it).
function flutterwaveCreateVirtualAccount(ownerEmail, ninOrBvn) {
  const fakeAcct = '90' + String(Math.floor(10000000 + Math.random() * 89999999));
  return { success: true, accountNumber: fakeAcct, bankName: 'Flutterwave-Titan MFB' };
}
...
route('POST', '/api/wallet/:ownerType/:ownerId/virtual-account', async (req, res, body, params) => {
  ...
  const result = flutterwaveCreateVirtualAccount(body.email, body.nin || body.bvn);
